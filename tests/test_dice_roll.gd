...
func test_die_fast() -> void:
	...
	check(d.verdict() == "A natural 20.", "a natural 20 that counted gets the gold word: %s" % d.verdict())
	...
	check(d.verdict() == "A natural 1.", "a natural 1 that missed says so: %s" % d.verdict())
