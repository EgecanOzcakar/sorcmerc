...
func _init() -> void:
	...
	root.add_child(game)
	# The linear debug run keeps the old door: the whole barracks, rested.
	var flag_was := OS.get_environment("SORCMERC_LINEAR_CAMPAIGN")
	...
	check(loaded != null, "the saved character loads into the linear run's party setup")
	check(loaded != null and loaded.hp_current == -1, "a new run resets HP to full (the usual -1 sentinel)")
	# Normal play founds a company instead (core/recruits.gd): nobody from the
	# barracks marches in, and the one hero made here is the founder.
	OS.set_environment("SORCMERC_LINEAR_CAMPAIGN", "")
	game.show_party_setup()
	screen = game._screen.get_child(0)
	check(screen.party.roster.is_empty(), "an open-world run starts with nobody: the founder is made, not loaded")
	check(Recruits.hire_only(screen.party), "...under the hiring rule")
	OS.set_environment("SORCMERC_LINEAR_CAMPAIGN", flag_was)
	game.queue_free()
	CharacterSave.delete("vera-hp-test")
