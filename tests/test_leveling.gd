...
func _catch_up_in_creator() -> void:
	...
	# A fresh run's preset starts at level 2: a level-3 build cut down, its
	# subclass waiting for the level that asks. A custom hero still starts at 1.
	scr.set_start_level(1)
	scr._load_preset("ilsa")
	check(scr.ch.level() == 2, "a preset starts a fresh run at level 2 (got %d)" % scr.ch.level())
