...
## New heroes start at level 2 (2026-09-24)

The owner's call: "make beginner characters start at level 2 so heartland
leveling is relevant." The Heartland is levels 1–3. A run used to start its
custom heroes at level 1 and its preset heroes at level 3. At 3 the party had
outgrown the Heartland on day one, since level 3 is its top (see the band pin
above). At level 2 the road home is worth one real level: the level that
brings the subclass, 200 XP or about six to seven open-country fights. The
Marches open once it's taken.

- **`Leveling.START_LEVEL = 2`.** `Party.join_level()` is the party's own level,
  never under it. The party screen hands that to the creator, so the first
  hero of a run is built out to level 2 and banks level 2's 100 XP. That XP is
  a gift, like every catch-up level, and none of it is lifetime XP. A hero
  who joins later still joins at the party's level.
- **The presets start at 2 too.** Vera, Pike and Ilsa are level-3 builds; the
  creator now loads them at `clampi(start_level, START_LEVEL, 3)` and tops up
  from there. Their subclass decision stays on the build unused and answers
  the level-3 choice when it arrives, so a new Vera is still a Champion, just
  not yet. `Presets.party_at()`, the ruler every sweep stands on, is untouched.
- The class step says so: "New heroes start at level 2, with 100 XP banked."

Measured (`tests/sweep_built.gd`, LEVELS=2,3, 80 seeds, easy, in band):
...
Level 2 sits where level 3 does, so no TIER moves.

### Still open

- The creator run on its own (`scenes/creator/creator.tscn`) still defaults to
  level 1 for custom heroes; its tests build level-1 characters on purpose.
  Only the party screen, the one path a run uses, floors at 2.
